/* general scattering simulator                                                -James Holton                11-21-17
...
//    printf("GOTHERE seed = %u\n",seed);
    ...
    printf("nearBragg assumption-free diffraction simulator - James Holton 11-21-17\n");
...
exit(9);
    ...
    #pragma omp parallel for private(\
        xpixel,ypixel,I,Fa,Fb,phase,suby,subz,Xdet,Ydet,stol,twotheta,\
        theta,vector,newvector,pixel_X,pixel_Y,pixel_Z,source_X,source_Y,source_Z,\
        source_to_atom_path,atom_to_pixel_path,airpath,lambda,\
        omega_Rsqr_pixel,\
        hdiv_tic,vdiv_tic,disp_tic,source_tic,S_X,S_Y,S_Z,S0_X,S0_Y,S0_Z,s_x,s_y,s_z,\
        test,i,j,k) \
    shared(sinimage,cosimage,floatimage,\
        atomX,atomY,atomZ,phsft,DWF,occ,Bfac,\
        xpixels,ypixels,pixels,subpixel,oversample,\
        fdet_vector,sdet_vector,pix0_vector,\
        curved_detector,distance,pixel,\
        sum,sumn,sumsqr,progress_meter,progress_pixel,progress_pixels,omega_sum,\
        point_pixel,coherent,atoms,\
        fluence,r_e_sqr,\
        max_I,max_I_x,max_I_y,\
        lambda0,dispersion,dispstep,dispsteps,\
        source_depth,far_source,source_distance,\
        steps,ddepth,depthstep,depthsteps,\
        hdiv,hdivrange,hdivstep,hdivsteps,vdiv,vdivrange,vdivstep,vdivsteps,round_div,\
        roi_xmin,roi_xmax,roi_ymin,roi_ymax,\
        debug,printout,printout_xpixel,printout_ypixel,stdout) default(none)
